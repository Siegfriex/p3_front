...
describe('Atlas query state contract', () => {
  it('canonicalizes answer types in A1 to A8 order and reports invalid values', () => {
    ...
    expect(result.canonicalSearch).toBe('status=active&types=A1%2CA2%2CA8&node=contract-node-001&view=nodes');
  ...
  it('normalizes invalid values without writing or inventing a selected node', () => {
    ...
    expect(result.state).toEqual({ status: 'all', types: [...ANSWER_TYPES], nodeId: null, view: 'nodes' });
  ...
  it('reset removes only Atlas parameters and default state serializes to an empty query', () => {
    ...
    expect(serializeAtlasQueryState({ status: 'all', types: [...ANSWER_TYPES], nodeId: null, view: 'nodes' })).toBe('');
  });

  it('builds a Story preview handoff URL without touching Story state', () => {
    expect(buildAtlasHrefFromPreview('complete', ['A7', 'A1'])).toBe('/atlas?status=complete&types=A1%2CA7&view=nodes');
  });
});
