...
export function AtlasExplorer({ bundle, query, queryIssues, fixtureProvenance, onQueryChange }: AtlasExplorerProps) {
  ...
  const announce = invalidNode
    ...
    : selectedNode
      ...
      : focusNodeId
        ? `${atlasNodeAccessibleName(bundle.nodes.find((node) => node.id === focusNodeId) ?? filteredNodes[0])} 포커스`
    ...
    <div className="space-y-5" onKeyDown={(event) => { if (event.key === 'Escape' && query.nodeId) clearSelection(); }}>
      ...
      {bundle.nodes.length === 0 ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]">
          <AtlasScene nodes={filteredNodes} selectedNodeId={query.nodeId} previewNodeId={previewNodeId} focusNodeId={focusNodeId} onSelectNode={selectNode} onPreviewNode={setPreviewNodeId} />
